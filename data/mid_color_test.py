...
class TestData(Dataset):
    ...
    def __getitem__(self,index):
        data = h5py.File(osp.join(self.data_path,self.data_list[index]))
        orig = data["orig"]
        pic = orig.value

        pic_gt = np.zeros([pic.shape[0] // 8,3,8,512,512])
        bayer_gt = np.zeros([pic.shape[0] // 8,8,512,512])
        for jj in range(pic.shape[0]):
            if jj % 8 == 0:
                meas_t = np.zeros([512,512])
                n = 0
            pic_t = pic[jj]
