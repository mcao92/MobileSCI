...
def save_image_color(out,gt,image_name):
    c,f,h,w = out.shape
    sing_out = out.transpose(2,1,3,0).reshape(h,f*w,c)
    sing_gt = gt.transpose(2,1,3,0).reshape(h,f*w,c)
    result_img = np.concatenate([sing_out,sing_gt],axis=0)
