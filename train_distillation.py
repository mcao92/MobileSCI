...
from models.efficientsci.network import Network_teacher
...
def train(args,network_student,network_teacher,logger,mask,mask_s,writer=None):
    ...
    for epoch in range(args.epochs):
        ...
        epoch_loss = 0
        network_student = network_student.train()
        ...
        if rank==0 and (epoch % args.save_model_step) == 0:
            ...
            if args.distributed:
                torch.save(network_student.module.state_dict(),osp.join(args.weight_path,"epoch_"+str(epoch)+".pth")) 
            else:
                torch.save(network_student.state_dict(),osp.join(args.weight_path,"epoch_"+str(epoch)+".pth")) 
        if rank==0 and args.test_flag:
            logger.info("epoch: {}, psnr and ssim test results:".format(epoch))
            if args.distributed:
                psnr_dict,ssim_dict = test(args,network_student.module,mask,mask_s,logger,writer=writer,epoch=epoch)
            else:
                psnr_dict,ssim_dict = test(args,network_student,mask,mask_s,logger,writer=writer,epoch=epoch)
...
if __name__ == '__main__':
    ...
    network_student = Network(color_ch=args.color_ch,depth=6,width=64).to(args.device)

    if rank==0 and args.pre_train_model is not None:
        logger.info("Load pre_train model...")
        model_dict = network_student.state_dict()
        ...
        network_student.load_state_dict(model_dict,strict=False)
    ...
    train(args,network_student,network_teacher,logger,mask,mask_s,writer)
    writer.close()
